...
function setupPagePill() {
    const save = q('save');
    const pageLabel = q('pageLabel');
    const app = q('text-editor-app');
    if (!save || !pageLabel || !app || !save.parentElement)
        return;
    ...
    pill.addEventListener('click', () => {
        ...
        const st = parse();
        if (Number.isFinite(st.current))
            inp.value = String(st.current);
        if (Number.isFinite(st.total))
            inp.max = String(st.total);
        const go = () => {
            const p = parse();
...
export function setupFormatDock() {
    ...
    if (fSize) {
        ...
        const step = (dir, icon, label) => {
            const b = mkBtn('dstep', label);
            b.appendChild(ph(icon));
            b.addEventListener('click', () => {
                if (fSize.disabled || fSize.value === '')
                    return;
                if (dir > 0)
                    fSize.stepUp();
                else
    ...
    if (outlineRow)
        moreSlot.appendChild(outlineRow);
    for (const [id, label] of MORE_FIELDS) {
        const row = sliderRow(id, label);
        if (row)
            moreSlot.appendChild(row);
    }
            ...
            b.addEventListener('click', () => {
                if (key === 'all')
                    ...
                    if (picked.has(key))
                        ...
                        picked.add(key);
                    if (picked.size === 0 || picked.size === 3)
                        picked = new Set(['all']);
                }
                applyScope();
            });
    ...
    const rebuildFonts = () => {
        const fam = q('fFamily');
        if (!(fam instanceof HTMLSelectElement))
            return;
    ...
    const visibleObjBtns = () => {
        const out = [];
        for (const id of OBJ_BTN_IDS) {
            const b = q(id);
            if (b instanceof HTMLButtonElement && !b.hidden)
                out.push(b);
        }
        return out;
    };
    const computeMode = () => {
        if (!inspector.hasAttribute('disabled'))
            return 'text';
        if (visibleObjBtns().length || !alignTools.hidden)
            return 'object';
        return 'idle';
    };
    const secChip = (spec) => {
    ...
    const proxyChip = (src) => {
        ...
        b.appendChild(icon ? icon.cloneNode(true) : ph('ph-circle'));
        b.addEventListener('click', () => src.click());
        return b;
    };
    const toolChip = (id, icon, label, onTap) => {
    ...
    const buildRail = () => {
        ...
        if (mode !== 'idle') {
            chipsWrap.appendChild(toolChip('done', 'ph-check', 'Done — back to tools', () => {
                closePanel();
                closeSheet();
                editBtn.click();
            }));
        ...
        if (mode === 'text') {
            ...
            chipsWrap.appendChild(toolChip('edit', 'ph-pencil-simple', 'Edit tool', () => {
                if (editBtn.classList.contains('on'))
                    openSec(SCOPE_SECTION);
                else {
                    closePanel();
                    applyScope();
                }
            }));
            chipsWrap.appendChild(toolChip('addText', 'ph-textbox', 'Add a text box', () => {
                closePanel();
                document
                    .querySelector('[data-tool="addText"]')
                    ?.click();
            }));
            chipsWrap.appendChild(toolChip('addImage', 'ph-image', 'Add an image', () => {
                closePanel();
                q('addImage')?.click();
            }));
            chipsWrap.appendChild(toolChip('find', 'ph-magnifying-glass', 'Find & replace', () => {
                closePanel();
                q('find')?.click();
            }));
        }
        if (!alignTools.hidden)
            chipsWrap.appendChild(secChip(MULTI_SECTION));
        if (!pStrokeRow.hidden)
            chipsWrap.appendChild(secChip(STROKE_SECTION));
        for (const b of visibleObjBtns())
            chipsWrap.appendChild(proxyChip(b));
        const delBtn = q('del');
    ...
    function markActive() {
        ...
        for (const c of chipsWrap.children) {
            if (!(c instanceof HTMLElement))
                continue;
            if (c.dataset.sec) {
                c.classList.toggle('on', c.dataset.sec === 'font' ? sheetOpen : c.dataset.sec === activeSec);
            }
    ...
    const publishDockHeight = () => {
        app.style.setProperty('--ecdock', `${Math.round(inspector.getBoundingClientRect().height)}px`);
    };
...
export function setupFindSheet() {
    ...
    const onStart = (e) => {
        ...
        vel = 0;
        const onGrab = e.target?.closest('.ecgrab');
    ...
    const onEnd = () => {
        ...
        if (dy > 90 || vel > 0.55) {
            ...
            findbar.animate([{ transform: `translateY(${dy}px)` }, { transform: 'none' }], { duration: 160, easing: 'ease-out' });
        }
    };
    ...
    new MutationObserver(() => {
        ...
        if (open && !findWasOpen) {
            findbar.animate([{ transform: 'translateY(40px)' }, { transform: 'none' }], { duration: 200, easing: 'ease-out' });
        }
        findWasOpen = open;
