  ...
  try {
    ...
    if (u32(0) === 0x74746366) {
      ...
      for (let fi = 0; fi < numFonts; fi++) {
        fontOffsets.push(u32(12 + fi * 4));
      }
    } else {
      fontOffsets.push(0);
    }
    for (const base of fontOffsets) {
      ...
      if (!cmap || cmap + 4 > bytes.length) continue;
      const nt = u16(cmap + 2);
      let sub = 0,
        fmt = -1,
        score = -1;
      ...
      const lookup4 = (cp) => {
        ...
        for (let s2 = 0; s2 < segX2; s2 += 2) {
          ...
          if (cp <= end) {
            ...
            const glyphOff = roBase + ro + (cp - start) * 2;
            if (glyphOff + 2 > bytes.length) return 0;
            const g = u16(glyphOff);
      ...
      for (const cp of cps) {
        if (
          ...
          continue;
        if ((fmt === 12 ? lookup12(cp) : lookup4(cp)) === 0) {
          allFound = false;
          break;
        }
      }
      ...
      (ctx, familyPtr, bold, italic, cps, n, outData, outSize) => {
        ...
        if (!bytes || !bytes.length) {
          ...
          if (hasCjk) {
            ...
            for (const cjkFam of cjkCandidates) {
              const b =
                PdfEngine.localFonts.get(cjkFam + sk) ||
                PdfEngine.localFonts.get(cjkFam);
              if (b && b.length) {
                bytes = b;
                break;
              }
